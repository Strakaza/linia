...
csp = {
    ...
    'img-src': ["'self'", "data:", "blob:", "https://*.mapbox.com", "https://liniabus.eu", "https://*.tile.openstreetmap.org", "https://tile.openstreetmap.org", "https://*.basemaps.cartocdn.com"],
...
@limiter.limit("20 per second") # Limite pour la recherche interactive
def api_search_stops():
    ...
    query_slug = simple_slugify(query_term)
    search_targets = {query_lower, query_term}
    if query_slug:
        search_targets.add(query_slug)
    
    for synonym, standard_name in SEARCH_SYNONYMS.items():
        syn_lower = synonym.lower()
        syn_slug = simple_slugify(synonym)
        if syn_lower.startswith(query_lower) or (query_slug and syn_slug.startswith(query_slug)):
