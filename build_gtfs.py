...
SOURCES = {
    ...
    "flixbus_tr": {
        "url": "https://gtfs.gis.flix.tech/gtfs_generic_turkey.zip",
        "prefix": "FLX_",
    },
    "blablacar": {
...
MAJOR_CITIES = [
    ...
    "Konya", "Kayseri", "Trabzon", "Diyarbakır", "Eskişehir", "Samsun", "Denizli", "Mersin",
    "Şanlıurfa", "Sanliurfa", "Bodrum", "Fethiye", "Marmaris", "Kuşadası", "Edirne"
]
...
def get_city_name(stop_name):
    ...
    cleaned_name_ascii = cleaned_name.replace('İ', 'I').replace('ı', 'i')
    lower_name = cleaned_name_ascii.lower()
    ...
    suffixes_to_remove = [
        ...
        " tcdd train station", " train station", " rest station", " other rest station",
        " otogar tesis", " otogar", " bus stop",
...
def main():
    ...
    if not all_data:
        logger.error("Aucune donnée disponible. Arrêt.")
        return

    unified_stops = pd.concat([data['stops'] for data in all_data.values() if not data['stops'].empty], ignore_index=True)
    unified_stop_times = pd.concat([data['stop_times'] for data in all_data.values() if not data['stop_times'].empty], ignore_index=True)

    city_stops, city_mapping = aggregate_by_city_select_flixbus(unified_stops)

    unified_trips = pd.concat([data['trips'] for data in all_data.values() if not data['trips'].empty], ignore_index=True)
    unified_routes = pd.concat([data['routes'] for data in all_data.values() if not data['routes'].empty], ignore_index=True)
    unified_shapes = pd.concat([data['shapes'] for data in all_data.values() if not data['shapes'].empty], ignore_index=True)
    ...
    final_routes = unified_routes[unified_routes['route_id'].isin(valid_routes)].drop_duplicates(subset=['route_id'], keep='first')
    ...
    if 'shape_id' in final_trips.columns and not final_trips['shape_id'].dropna().empty:
        ...
        final_shapes = unified_shapes[unified_shapes['shape_id'].isin(valid_shapes)].drop_duplicates(subset=['shape_id', 'shape_pt_sequence'], keep='first')

    agency_list = [data['agency'].head(1) for data in all_data.values() if not data['agency'].empty]
    final_agency = pd.concat(agency_list, ignore_index=True).drop_duplicates(subset=['agency_name'], keep='first')
